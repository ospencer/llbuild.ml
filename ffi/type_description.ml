...
let llb_build_key : llb_build_key structure typ = structure "llb_build_key_t_"
...
let llb_fs_file_info_mod_time =
  field llb_fs_file_info "mod_time" llb_fs_timestamp
...
let llb_bsi_db_path = field llb_buildsystem_invocation "dbPath" string
...
let llb_bsi_qos = field llb_buildsystem_invocation "qos" int
...
let llb_bsd_context = field llb_buildsystem_delegate "context" (ptr void)
...
let llb_bsd_command_found_discovered_dependency =
  field llb_buildsystem_delegate "command_found_discovered_dependency"
    (static_funptr
       (ptr void
       @-> ptr llb_buildsystem_command
       @-> string @-> int @-> returning void))
...
let llb_bsd_command_cannot_build_output_due_to_missing_inputs =
  field llb_buildsystem_delegate
    ...
    (static_funptr
       (ptr void
       @-> ptr llb_buildsystem_command
       @-> ptr (ptr llb_build_key)
       @-> ptr (ptr llb_build_key)
       @-> uint64_t @-> returning void))
...
let llb_bsd_choose_command_from_multiple_producers =
  field llb_buildsystem_delegate "choose_command_from_multiple_producers"
    (static_funptr
       (ptr void
       @-> ptr (ptr llb_build_key)
       @-> ptr (ptr llb_buildsystem_command)
       @-> uint64_t
       @-> returning (ptr llb_buildsystem_command)))

let llb_bsd_cannot_build_node_due_to_multiple_producers =
  field llb_buildsystem_delegate "cannot_build_node_due_to_multiple_producers"
    (static_funptr
       (ptr void
       @-> ptr (ptr llb_build_key)
       @-> ptr (ptr llb_buildsystem_command)
...
let llb_bsd_command_process_finished =
  field llb_buildsystem_delegate "command_process_finished"
    (static_funptr
       ...
       @-> ptr llb_buildsystem_command
       @-> ptr llb_buildsystem_process
       @-> ptr llb_buildsystem_command_extended_result
       @-> returning void))
...
let llb_bsd_should_resolve_cycle =
  field llb_buildsystem_delegate "should_resolve_cycle"
    (static_funptr
       (ptr void
       @-> ptr (ptr llb_build_key)
       @-> uint64_t @-> ptr llb_build_key @-> int @-> returning uint8_t))
...
let llb_buildsystem_tool_delegate : llb_buildsystem_tool_delegate structure typ
    =
  structure "llb_buildsystem_tool_delegate_t_"

let llb_bstd_context = field llb_buildsystem_tool_delegate "context" (ptr void)

let llb_bstd_create_command =
  field llb_buildsystem_tool_delegate "create_command"
    (static_funptr
       (ptr void @-> ptr llb_data @-> returning (ptr llb_buildsystem_command)))
...
let llb_bsecd_start =
  field llb_buildsystem_external_command_delegate "start"
    (static_funptr
       ...
       @-> llb_task_interface @-> returning void))
...
let llb_bsecd_provide_value =
  field llb_buildsystem_external_command_delegate "provide_value"
    (static_funptr
       ...
       @-> ptr llb_buildsystem_command
       @-> ptr llb_buildsystem_interface
       @-> llb_task_interface @-> ptr llb_build_value @-> uintptr_t
       @-> returning void))
...
let llb_bsecd_is_result_valid =
  field llb_buildsystem_external_command_delegate "is_result_valid"
    (static_funptr
       (ptr void
       @-> ptr llb_buildsystem_command
       @-> ptr llb_build_value @-> returning bool))
...
let llb_bvft_seconds = field llb_build_value_file_timestamp "seconds" uint64_t
